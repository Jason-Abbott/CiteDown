...
// https://github.com/ayushgupta2209/SqliteIntegration/blob/master/SqliteIntegration/SqliteDbStore.swift
...
class SQLiteSource {
    ...
    let oslog = OSLog(subsystem: "toba", category: "sqliteintegration")
    ...
    init() {
        do {
            do {
                ...
                os_log("URL: %s", dbPath.absoluteString)
            } catch {
    ...
    func openDB() throws {
        if sqlite3_open(dbPath.path, &db) != SQLITE_OK { // error mostly because of corrupt database
            os_log("error opening database at %s", log: oslog, type: .error, dbPath.absoluteString)
//            deleteDB(dbURL: dbURL)
            throw SqliteError(message: "error opening database \(dbPath.absoluteString)")
        }
    }
...
// Exception during a SQLite Operation
